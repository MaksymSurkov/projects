(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        function fixIOSViewportChrome() {
            const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) || navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1;
            if (!isIOS || !window.visualViewport) return;
            const root = document.documentElement;
            root.classList.add("ios-viewport-fix");
            const updateViewportInsets = () => {
                const viewport = window.visualViewport;
                const topInset = Math.max(0, viewport.offsetTop || 0);
                const bottomInset = Math.max(0, window.innerHeight - viewport.height - topInset);
                const cappedTopInset = Math.min(topInset, 120);
                const cappedBottomInset = Math.min(bottomInset, 140);
                root.style.setProperty("--ios-browser-inset-top", `${cappedTopInset}px`);
                root.style.setProperty("--ios-browser-inset-bottom", `${cappedBottomInset}px`);
            };
            updateViewportInsets();
            window.addEventListener("load", updateViewportInsets, {
                passive: true
            });
        ...
        const mousePrlxEnabled = window.matchMedia("(hover: hover) and (pointer: fine)").matches;
        flsModules.mousePrlx = mousePrlxEnabled ? new MousePRLX({}) : null;
